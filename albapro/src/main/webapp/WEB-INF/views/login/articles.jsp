...
<body>
<div class="header">
	 <a href= "/proalba"><img src="resources/images/weblogo.png" class="logo1" /></a>
...
본 약관은 주식회사 프로알바(이하 회사라 함)이 제공하는 프로알바 이용과 관련하여, 회사와 회원과의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
①본 약관에서 사용하는 용어의 정의는 다음과 같습니다.
    ...
    1. 서비스 : 클라우드 기반 간편 전자계약 서비스 프로알바을 말합니다.
    2. 유료서비스 : 회사가 제공하는 서비스 중 회원이 회사에 일정 금액을 결제하거나, 회원이 회사 또는 제3자와의 
       거래 내지 약정 조건을 수락하는 경우에 이용할 수 있는 회사의 서비스를 말합니다.
    3. 이용자 : 서비스를 이용하는 회원 또는 비회원을 말합니다.
    4. 회원 : 서비스에 접속하여 본 약관에 따라 회사와 이용계약을 체결하고 회사가 제공하는 서비스를 이용하는 자
       를 말합니다.
    5. 비회원 : 서비스에 가입하지 않고 서비스가 제공하는 정보 및 서비스를 이용하는 자를 말합니다.
    6. 유료회원 : 유료서비스를 이용하는 회원을 말합니다.
    7. 개인정보 : 서비스를 이용하기 위해 회원이 입력하여 서비스에 기재되는 이름, 이메일 주소 등 회원의 개인정보
       를 말합니다.
    8. 서명 요청자 : 서명 참여자에게 문서에 대한 전자서명을 요청하는 회원을 말합니다.
    9. 서명 참여자 : 서명 요청자로부터 전자서명을 요청받은 문서에 전자서명을 하는 이용자를 말합니다.
...
③ 회사는 가입신청자의 신청에 대하여 서비스 이용을 승낙함을 원칙으로 합니다. 다만, 회사는 다음 각 호에 해당하는 신청에 대하여는 승낙을 하지 않거나 사후에 이용계약을 해지할 수 있습니다.
    ...
    4. 만 14세 미만 아동이 정보통신망 이용촉진 및 정보보호 등에 관한 법률에서 정한 개인정보 입력 시 법정대리인
       의 동의를 얻지 아니한 경우
...
① 회원으로 가입하여 서비스를 이용하고자 하는 이용자는 이메일, 이름 등 정보를 제공하고, 이메일 인증 또는 소셜 인증(네이버, 구글, 페이스북)을 하여야 합니다.
...
① 회원은 서비스의 원활한 이용 및 회원의 정보보호, 서비스 이용안내 등의 편의를 위하여 이용자가 선정한 이메일 주소를 계정으로 사용합니다. 다만, 회사는 회원의 계정이 반사회적이거나 미풍양속을 해치거나 또는 운영자로 오인할 우려가 있는 경우 등에는 해당 계정의 사용을 거부하거나 제한할 수 있습니다
...
① 회원은 홈페이지에 개제된 회사 연락처에 연락하거나 서비스 내 탈퇴 기능을 통해 탈퇴 신청을 할 수 있으며, 회사는 관련법령 등에서 정하는 바에 따라 이를 즉시 처리하여야 합니다.

② 다음 각호에 해당하는 경우를 제외하고, 회원의 모든 정보는 탈퇴 시점 이후 바로 삭제되며 복구할 수 없습니다.

    1. 서명완료된 문서 및 각종 정보(단, 서명 요청자 및 서명 참여자 전원의 동의가 있을 경우 삭제 가능)
...
① 서명 요청자가 서명 참여자에게 서명을 요청하는 시점에 서명 요청 1건이 차감됩니다.

② 서명 취소, 거절, 요청 취소 시 서명 요청 건은 반환되지 않습니다.

 

제17조(전자문서 법적효력에 대한 확인)
...
① 회사가 유료회원의 이용신청을 승낙한 때(일부 서비스의 경우, 이용 신청 접수 후 사전 공지한 지정된 일자)로부터 유료서비스는 개시되며, 회사의 기술적 사유 등 기타 사정에 의하여 유료서비스를 개시할 수 없는 경우에는 제9조의 방법에 따라 회원에게 사전 공지합니다.
...
- 건별 이용권 : 건별 이용권 사용 기간이 제한이 없는 유료서비스를 말합니다.
...
① 회사는 회원이 유료서비스 변경을 신청할 경우 즉시 반영합니다.

② 회원이 월 요금제 변경을 신청할 경우 변경신청 전 사용하지 않은 서명 요청 건은 환불되지 않습니다.
...
제23조 (청약 철회 및 서비스 이용계약의 해제 • 해지)

① 유료회원은 유료서비스 공급일 이후 제공되는 서명 요청 건을 사용하지 않았을 경우 7일 이내에 회사에 결제 취소(청약 철회)를 요청할 수 있습니다.

② 유료회원이 유료서비스 공급일 이후 제공되는 서명 요청 건을 사용하지 않았더라도 청약 철회가 가능한 기간(유료서비스 공급일로부터 7일 이내)을 경과하여 청약 철회를 신청하면 환불은 불가능합니다.
...
⑤ 회사는 유료회원이 청약 철회, 해지/해제 신청을 확인 후 환불 금액이 있을 경우, 원칙적으로 유료회원의 해당 의사표시를 수령한 날로부터 3영업일 이내에 결제수단 별 사업자에게 대금의 청구 정지 내지 취소를 요청하고, 유료회원이 결제한 동일 결제수단으로 환불함을 원칙으로 합니다. 단, 회사가 사전에 유료회원에게 전자메일, 서비스 홈페이지로 공지한 경우 및 아래의 각 호의 경우와 같이 개별 결제 수단별 환불 방법, 환불 가능 기간 등이 차이가 있을 수 있습니다.

    1. 신용카드 등 수납 확인이 필요한 결제수단의 경우에는 수납 확인일로부터 3영업일 이내
    2. 결제수단 별 사업자가 회사와의 약정을 통하여 사전에 대금 청구 정지 내지 결제 취소 가능 기한 등을 미리 정하
       여 둔 경우로 해당 기한을 지난 환불의 경우
    3. 유료회원이 환불 처리에 필요한 정보 내지 자료를 회사에 즉시 제공하지 않는 경우(현금 환불 시 신청인의 계좌 
...
⑩ 유료서비스를 이용 중인 유료회원이 탈퇴하는 경우 유료서비스는 즉시 해지되며 환불은 불가능합니다. 이 경우 유료회원의 정보와 서비스이용내용은 관련 법령이 정하는 경우를 제외하고는 약관 및 이 약관에 따라 처리됩니다.

 

제24조 (유료서비스 가액 변경)

회사는 운영상, 기술상의 필요에 따라 공급하는 유료서비스 가액을 변경할 수 있으며, 제9조에서 정한 방법에 따라 유료회원에게 사전공지합니다.
...
④ 회사는 과오금의 세부 환불절차 및 기타 사항에 대하여 다음과 같이 「콘텐츠이용자보호지침」이 정하는 바에 따릅니다.
    ...
    1. 회사나 회원이 과오금의 발생사실을 안 때에는 이메일, 서비스 홈페이지 등 회사가 제시한 방법에 따라 상대방
       에게 통보
    2. 회사는 회원에게 환불에 필요한 정보를 요청(회원 성명, 결제증빙서류, 전화번호, 환불 요청 계좌 등)
    3. 회원은 환불에 필요한 2호의 정보를 회사에 제공
    4. 회사는 이용자의 정보 제공일로부터 7일 이내 환불 처리 (회원의 명시적 의사표시가 있는 경우 차회 요금에서 
       상계)
...
① 이용자는 아래 각 호에 해당하는 최소 사용 환경에서 서비스를 이용해야 합니다.
    ...
    2. 브라우저 : Internet Explorer(11 이상), Edge(25.1 이상), Chrome(49 이상), Safari(9.0 이상) Opera(37 이상), 
    Firefox(43.0 이상)
    3. 쿠키 : 모든 쿠키 허용
...
① 회원은 회사에서 제공하는 서비스를 본래의 이용 목적 이외의 용도로 사용하거나 다음 각 호에 해당하는 행위를 해서는 안됩니다.
    ...
    1. 가입신청 또는 정보 변경을 목적으로 회사에 개인정보 등록시 실명이 아닌 정보 또는 다른 사람의 정보를 사용
       하거나 허위 사실을 기재하는 행위
    2. 타인으로 가장하거나 타인과의 관계를 허위로 명시하는 행위, 다른 회원의 계정 및 비밀번호를 도용, 부정하게 
       사용하는 행위
    3. 알려지거나 알려지지 않은 버그를 악용하여 서비스를 이용하는 행위
    4. 회사 및 제3자의 명예를 훼손하거나 업무를 방해하거나 회사 및 제3자에게 손해를 가하는 행위
    5. 회사의 지적재산권, 제3자의 지적재산권, 초상권 등 기타 권리를 침해하거나 회사의 승인을 받지 않고 다른 회원
       의 개인정보를 수집, 저장, 유포, 게시하는 행위
    6. 제3자를 기망하여 이득을 취하거나 회사가 제공하는 서비스를 불건전하게 이용하거나 하여 제3자에게 피해를 
       주는 행위
    7. 회사로부터 특별한 권리를 부여받지 않고 사이트를 변경하거나 사이트에 다른 프로그램을 추가 또는 삽입하거
       나 서버를 해킹, 역설계, 소스코드의 유출 및 변경, 별도의 서버를 구축하거나 웹사이트의 일부분을 임의로 변경
       또는 도용하여 회사를 사칭하는 행위
    8. 회사의 직원이나 운영자를 가장, 사칭하거나 또는 타인의 명의를 도용하여 문서를 등록하거나 메일을 발송하는 
       행위
...
④ 회사는 제1항, 제2항 및 다음 각 호의 어느 하나에 해당하는 행위의 구체적인 유형을 운영 정책에서 정할 수 있으며, 회원은 이를 준수할 의무가 있습니다.
    ...
    2. 기타 회원의 서비스 이용에 대한 본질적인 권리를 침해하지 않는 범위 내에서 회사가 운영상 필요하다고 인정되
       는 사항
 

제29조 (손해배상)

① 회사가 제공하는 유료서비스의 하자 등에 의하여 회원에게 피해가 발생한 경우, 회원에 대한 피해보상의 기준, 범위, 방법 및 절차에 관한 사항은 디지털콘텐츠이용자보호지침에 따릅니다.
...
회원이 서명 요청자로서 서명 참여자에게 전자서명을 요청할 경우 전자서명 이용약관이 함께 적용됩니다.</textarea>
</div>
<div class="footer">
